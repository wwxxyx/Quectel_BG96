...
/* Uart Dbg */
qapi_UART_Handle_t m2m_dbg_uart_handler;
...
void m2m_uart_dbg_init(void)
{
	...
	status = qapi_UART_Open(&m2m_dbg_uart_handler, QAPI_UART_PORT_002_E, &uart_cfg);
	if (QAPI_OK != status)
	{
		return;
	}
	status = qapi_UART_Power_On(m2m_dbg_uart_handler);
}
...
void m2m_uart_debug_print(const char* fmt, ...) 
{
	...
	qapi_UART_Transmit(m2m_dbg_uart_handler, quec_dbg_buffer, strlen(quec_dbg_buffer), NULL);
    qapi_UART_Transmit(m2m_dbg_uart_handler, "\r\n", strlen("\r\n"), NULL);
...
qapi_Status_t lwm2m_ext_read(int obj_id, int inst_id, int res_id)
{
  	...
  	else
	{
		...
		while(object->instance_info != NULL)
		{
			...
			M2M_UART_PRINT("Read on / %d / %d ", obj_id, inst_id);
...
qapi_Status_t lwm2m_ext_write(int obj_id, int inst_id, int res_id, char *value)
{
	...
	M2M_UART_PRINT("Write on / %d / %d / %d", obj_id, inst_id, res_id);
	switch(resource->type)
	{
		case QAPI_NET_LWM2M_TYPE_STRING_E:
		{
...
qapi_Status_t lwm2m_ext_exec(int obj_id, int inst_id, int res_id)
{
	...
	M2M_UART_PRINT("Execute on / %d / %d / %d", obj_id, inst_id, res_id);
...
{ 
	M2M_UART_PRINT("Object id is %d \n", LWM2M_GENERIC_SENSOR_OBJECT_ID);
	M2M_UART_PRINT("Object is multi-instance \n");
	M2M_UART_PRINT("Res_id   Res_name 				  Res_instances  Mandatory");
	M2M_UART_PRINT("	Type	Operation Description\n");
	M2M_UART_PRINT("0 	   Sensor value 			  Single		 Yes	  ");
	M2M_UART_PRINT("	Integer R		  Last or current measured sensor value\n");
	M2M_UART_PRINT("1 	   Sensor units 			  Single		 No 	  ");
	M2M_UART_PRINT("	String	R		  Measurement unit definition e.g. Celcius for tempwerature.\n");
	M2M_UART_PRINT("2 	   Min measured value		  Single		 No 	  ");
	M2M_UART_PRINT("	Float	R		  Minimum value measured since power on or reset \n");
	M2M_UART_PRINT("3 	   Max measured value		  Single		 No 	  ");
	M2M_UART_PRINT("	Float	R		  Maximum value measured since power on or reset \n");
	M2M_UART_PRINT("4 	   Min range value			  Single		 Yes	  ");
	M2M_UART_PRINT("	Float	R/W 	  Minimum threshold value. \n");
	M2M_UART_PRINT("5 	   Max range value			  Single		 Yes	  ");
	M2M_UART_PRINT("	Float	R/W 	  Maximum threshold value. \n");
	M2M_UART_PRINT("6  Reset min and max mesaured value Single		 No 	  ");
	M2M_UART_PRINT("	Opaque	E		  Reset minimum and maximum measured value. \n");
	M2M_UART_PRINT("7 	   Sensor state 			  Single		 Yes	  ");
	M2M_UART_PRINT("	Boolean R/W 	  Sensor state on/off.. \n");
	M2M_UART_PRINT("8 	   Mean value				  Single		 No 	  ");
	M2M_UART_PRINT("  Integer R		   Mean measured value. \n");
}
