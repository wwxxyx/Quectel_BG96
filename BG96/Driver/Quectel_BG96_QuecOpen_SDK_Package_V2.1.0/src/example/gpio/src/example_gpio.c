...
int quectel_task_entry(void)
{
	...
	IOT_INFO("QT# ### Test pin is %d, gpio_id is %d ###", g_test_pin_num, gpio_map_tbl[g_test_pin_num].gpio_id);
	...
	while(1)
	{
		...
		IOT_DEBUG("QT# Set %d QAPI_GPIO_LOW_VALUE_E status = %d", g_test_pin_num, status);
		...
		IOT_DEBUG("QT# Set %d QAPI_GPIO_HIGH_VALUE_E status = %d", g_test_pin_num, status);
		qapi_Timer_Sleep(2, QAPI_TIMER_UNIT_SEC, true);

		status = qapi_TLMM_Release_Gpio_ID(&tlmm_config[g_test_pin_num], gpio_id_tbl[g_test_pin_num]);
		IOT_DEBUG("QT# release %d status = %d", g_test_pin_num, status);
	...
	while (1)
	{
		...
		IOT_DEBUG("QT# read %d status = %d, level=%d", g_test_pin_num, status, level);
		status = qapi_TLMM_Release_Gpio_ID(&tlmm_config[g_test_pin_num], gpio_id_tbl[g_test_pin_num]);
		IOT_DEBUG("QT# release status = %d", status);
