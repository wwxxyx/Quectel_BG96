...
void mqtt_uart_dbg_init(void)
{
	...
	status = qapi_UART_Open(&mqtt_dbg_uart_handler, QAPI_UART_PORT_002_E, &uart_cfg);
	if (QAPI_OK != status)
	{
		return;
	}
	status = qapi_UART_Power_On(mqtt_dbg_uart_handler);
}
...
void mqtt_uart_debug_print(const char* fmt, ...) 
{
	...
	qapi_UART_Transmit(mqtt_dbg_uart_handler, quec_dbg_buffer, strlen(quec_dbg_buffer), NULL);
...
void mqtt_cli_connect_callback(qapi_Net_MQTT_Hndl_t mqtt, int32 reason)
{
  ...
  {
    MQTT_UART_DBG("MQTT Connected, reaon code=%d\n", reason);
  }
  else 
  {
     MQTT_UART_DBG("MQTT Connection failed, reaon code=%d\n", reason);
     mqtt_cli_disconnect();
  }
}
...
void mqtt_cli_connect(void)
{
  ...
  MQTT_UART_DBG("IP for remote: %s Port: %d\n", inet_ntop(AF_INET, &sin4->sin_addr.s_addr, buff, sizeof(buff)), _ntohs(sin4->sin_port));
...
/* test with Telit Iot Platform */
...
#if 0
  sin4->sin_addr.s_addr = inet_addr(CLI_MQTT_SVR);  
#else
    ipaddr.type = AF_INET;
  ...
  if(ssl_enabled == 1)
  {
      mqttdemo_cfg.ca_list = ROOT_CA_FILENAME;
      mqttdemo_cfg.cert = CERTIFICATE_FILENAME;
