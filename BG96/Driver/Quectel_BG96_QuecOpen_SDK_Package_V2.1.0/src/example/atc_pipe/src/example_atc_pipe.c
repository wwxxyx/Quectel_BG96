...
static void callback_atc_pipe_0(qapi_at_pipe_data_t *data)
{
	...
	qt_uart_dbg(uart1_conf.hdlr, "[DBG][ATC-1]Current Data: %s, %d", data->data, data->len);
	strcat(at_cmd_rsp, data->data);

//	IOT_DEBUG("QT # callback_atc_pipe data # %s", data);
//	IOT_DEBUG("QT # callback_atc_pipe at_cmd_rsp # %s", at_cmd_rsp);
	qt_uart_dbg(uart1_conf.hdlr, "[DBG][ATC-1]at_cmd_rsp len: %d", strlen(at_cmd_rsp));
...
static void callback_atc_pipe_1(qapi_at_pipe_data_t *data)
{
	...
	qt_uart_dbg(uart1_conf.hdlr, "[DBG][ATC-2]Current Data: %s, %d", data->data, data->len);
	strcat(at_cmd_rsp, data->data);

//	IOT_DEBUG("QT # callback_atc_pipe data # %s", data);
//	IOT_DEBUG("QT # callback_atc_pipe at_cmd_rsp # %s", at_cmd_rsp);
	qt_uart_dbg(uart1_conf.hdlr, "[DBG][ATC-2]at_cmd_rsp len: %d", strlen(at_cmd_rsp));
...
)
{
	...
	/* Open first ATC port */
 	status = qapi_QT_Apps_AT_Port_Open(QAPI_AT_PORT_0, &stream_id_0, callback_atc_pipe_0);
	...
	/* Open second ATC port */
 	status = qapi_QT_Apps_AT_Port_Open(QAPI_AT_PORT_1, &stream_id_1, callback_atc_pipe_1);
	if(status != QAPI_QT_ERR_OK)
	{
