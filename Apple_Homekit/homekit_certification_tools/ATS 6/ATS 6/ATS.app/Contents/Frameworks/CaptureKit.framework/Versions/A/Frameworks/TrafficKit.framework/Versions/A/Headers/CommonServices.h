...
// cbrimhall: project doesn't build if unit tests are included
...
// APPLE_INTERNAL -- True if building for Apple internal code. Apple internal code is stripped out for 3rd parties.

#if( !defined( APPLE_INTERNAL ) )
	#define APPLE_INTERNAL			1
#endif

// APPLE_EXTERNAL -- True if building for code given outside of Apple.

#if( !defined( APPLE_EXTERNAL ) )
	#define APPLE_EXTERNAL			0
#endif

// COMMON_SERVICES_NO_CORE_SERVICES -- 1 to not include any Mac/iOS-specific pieces even when building on those platforms.

#if( !defined( COMMON_SERVICES_NO_CORE_SERVICES ) )
	#define COMMON_SERVICES_NO_CORE_SERVICES		0
#endif

// CU_BUILDING_FOR_TEST_ENVIRONMENT - 1 if we're building for unit testing, etc.

#if( !defined( CU_BUILDING_FOR_TEST_ENVIRONMENT ) )
	#define CU_BUILDING_FOR_TEST_ENVIRONMENT		0
#endif
...
#if( defined( __BLOCKS__ ) && __BLOCKS__ && !COMMON_SERVICES_NO_CORE_SERVICES )
	#define COMPILER_HAS_BLOCKS		1
#else
	#define COMPILER_HAS_BLOCKS		0
#endif

#if( defined( __OBJC__ ) && __OBJC__ && !COMMON_SERVICES_NO_CORE_SERVICES )
...
// CUDefer - Like Swift's "defer" keyword to run code when the current scope exits.
//
// Example:
//
...
#if( COMPILER_CLANG && COMPILER_HAS_BLOCKS )
	typedef void ( ^CUDeferBlock )( void );
	static inline void _CUDeferCallback( CUDeferBlock *inBlock ) { ( *inBlock )(); }
	#define _CUDeferMerge( a, b )	a##b
	#define _CUDeferNamed( a )		_CUDeferMerge( __CUDeferVar_, a )
	#define CUDefer __extension__ __attribute__( ( cleanup( _CUDeferCallback ), unused ) ) CUDeferBlock _CUDeferNamed( __COUNTER__ ) = ^
#endif

// CUGuardLet -- Like Swift's "guard let" for an auto-type'd constant with a test, but without needing "else".
// 
// Example:
//
// CUGuardLet( str, NSDictionaryGetNSString( dict, @"x", NULL ) )
// {
//		NSLog( "Inside %@", str );
//		return;
// }
...
#define CUGuardLet( CONSTANT, VALUE )	__auto_type CONSTANT = (VALUE); if( (CONSTANT) ) {} else

// CUIfLet -- Like Swift's "if let" for a scoped auto-type'd constant with a test.
// 
// Example:
//
// CUIfLet( str, NSDictionaryGetNSString( dict, @"x", NULL ) )
// {
//		NSLog( "%@", str );
// }

#define CUIfLet( CONSTANT, VALUE ) \
...
// CULet -- Like Swift's "let" keyboard for an auto-type'd constant.
// 
// Example:
//
// CULet x = [[NSMutableDictionary <NSString *, NSNumber *> alloc] init];
// x[ @"x" ] = @123;
// x = [[NSMutableDictionary <NSString *, NSNumber *> alloc] init]; // Compile error.

#if( defined( __cplusplus ) )
	#define CULet		auto const
#else
	#define CULet		const __auto_type
#endif

// CUVar -- Like Swift's "var" keyboard for an auto-typed variable.
//
// Example:
//
// CUVar x = [[NSMutableDictionary <NSString *, NSNumber *> alloc] init];
// x[ @"x" ] = @123;
// x = [[NSMutableDictionary <NSString *, NSNumber *> alloc] init]; // No compile error.

#if( defined( __cplusplus ) )
	#define CUVar		auto
#else
	#define CUVar		__auto_type
#endif
...
// STATIC_PARAM -- C99 feature to specify parameter is a fixed size (e.g. void MyFunc( char inStr[ static 16 ] )).
// C++ doesn't support this so we have to work around by stripping it out.

#if( !defined( STATIC_PARAM ) )
	#if( defined( __cplusplus ) )
		#define STATIC_PARAM
	#else
		#define STATIC_PARAM		static
	#endif	
#endif
...
// Deprecation usage -- Silents warnings about deprecated code between the begin and end.

#if( COMPILER_CLANG )
	#define cu_deprecated_usage_begin() \
...
#if( COMPILER_CLANG )
	#define cu_deprecated_usage_end() \
...
// NonnullCast
//
// Performs a type-safe cast from a nullable pointer to a nonnull pointer.
// You must check for null before using this. This will abort the process if used with a null pointer.

#if( COMPILER_OBJC )
	@interface CUNonnullAccessor <__covariant Type>
	- (nonnull Type) asNonNull;
	@end
	
	#define NonnullCast( X ) \
		__extension__ \
		({ \
			id											obj = (X); \
			CUNonnullAccessor <__typeof__( (X) )> *		type; \
			\
			require_fatal( obj, "Nonnull cast with null object" ); \
			( __typeof__( type.asNonNull ) ) obj; \
		})
#endif
...
// STATIC_ANALYZER_NORETURN -- Tells the static analyzer to assume a function doesn't return (e.g. assertion handlers).
...
#if( !defined( TARGET_OS_EFI ) )
// EFI (Extensible Firmware Interface)
	#if( !TARGET_OS_OVERRIDE )
		#if( defined( EFI32 ) || defined( EFI64 ) || defined( EFIX64 ) )
			#define	TARGET_OS_EFI			1
			
			// We may be building on a Mac, but we're not building Mac code so force it off.
			
			#undef	TARGET_OS_MAC
			#define TARGET_OS_MAC			0
			
			// Force Windows conditionals off because EFI uses Windows compilers and looks like Windows otherwise.
...
#if( !defined( TARGET_OS_RTXC_DEVICE ) )
// RTXC
	#if( !TARGET_OS_OVERRIDE )
		#if( defined( t_env_os ) && defined( t_env_os_RTXC ) && ( t_env_os == t_env_os_RTXC ) )
			#define TARGET_OS_RTXC_DEVICE		1
		#else
			#define TARGET_OS_RTXC_DEVICE		0
		#endif
	#else
		#define TARGET_OS_RTXC_DEVICE 			0
	#endif
#endif
#if( !defined( TARGET_OS_RTXC_SIMULATOR ) )
	#if( !TARGET_OS_OVERRIDE )
		#if( defined( t_env_os ) && defined( t_env_SIMULATED ) && ( t_env_os == t_env_SIMULATED ) )
			#define TARGET_OS_RTXC_SIMULATOR	1
		#else
			#define TARGET_OS_RTXC_SIMULATOR	0
		#endif
	#else
		#define TARGET_OS_RTXC_SIMULATOR 		0
	#endif
#endif
#if( !defined( TARGET_OS_RTXC ) )
	#if( !TARGET_OS_OVERRIDE )
		#define TARGET_OS_RTXC				( TARGET_OS_RTXC_DEVICE || TARGET_OS_RTXC_SIMULATOR )
	#else
		#define TARGET_OS_RTXC 				0
	#endif
#endif
#if( TARGET_OS_RTXC )
	#undef  TARGET_OS_DARWIN
	#define TARGET_OS_DARWIN				0 // Force Darwin off since the RTCX Simulator uses Darwin compilers, etc.
#endif
...
// Windows
		...
		#else
			#if( defined( _WIN32 ) && !TARGET_OS_EFI && !TARGET_OS_WINDOWS_KERNEL )
...
// TARGET_MACH -- Any Mach-based OS.
	...
	#if( defined( __MACH__ ) && __MACH__ && !TARGET_OS_EFI ) // EFI can build with Mach-o compilers so exclude EFI.
...
// TARGET_OS_DARWIN_REAL -- Any variant of Darwin except when we're building on Darwin to simulate a non-Darwin system.
// This is used to build and test POSIX-compatible versions of code from a Darwin system.

#if( !defined( TARGET_OS_DARWIN_REAL ) )
	#if( TARGET_OS_DARWIN && !COMMON_SERVICES_NO_CORE_SERVICES )
		#define TARGET_OS_DARWIN_REAL	1
	#else
		#define TARGET_OS_DARWIN_REAL	0
	#endif
#endif
...
#if( TARGET_OS_EFI || TARGET_KERNEL || TARGET_OS_PALM || TARGET_OS_RTXC || TARGET_OS_THREADX || TARGET_OS_WINDOWS || TARGET_OS_VXWORKS )
...
// TARGET_HAS_EFI_LIBCLITE

#if( !defined( TARGET_HAS_EFI_LIBCLITE ) )
	#if( defined( TARGET_EFI ) && TARGET_EFI )
		#define TARGET_HAS_EFI_LIBCLITE		1
	#else
		#define TARGET_HAS_EFI_LIBCLITE		0
	#endif
#endif
...
#if( TARGET_OS_EFI || TARGET_KERNEL )
...
#if( TARGET_OS_EFI && !TARGET_HAS_EFI_LIBCLITE )
	#define TARGET_HAS_FLOATING_POINT_SUPPORT		0
#else
...
// TARGET_HAS_SOCKETS -- Has BSD-style socket API support.
	...
	#if( !TARGET_PLATFORM_WICED && !TARGET_OS_EFI )
...
#endif
#if( ( COMPILER_VISUAL_CPP >= 1400 ) && !TARGET_OS_EFI )
...
#if( !TARGET_KERNEL && !TARGET_OS_EFI )
	#include <stddef.h>
#endif
...
// Darwin non-kernel
	...
	#endif
	#if( APPLE_INTERNAL && !COMMON_SERVICES_NO_CORE_SERVICES )
		#if( __has_include( <nw/private.h> ) )
			#include <nw/private.h>
		#elif( __has_include( <network/tcp_connection.h> ) )
...
// EFI
	...
	#if( TARGET_HAS_EFI_LIBCLITE )
		#include <LibCLite/LibCLite.h>
	#elif( defined( __GNUC__ ) )
		#include <limits.h>
		#include <stddef.h>
		#include <stdlib.h>
...
#if 0
#pragma mark == Target OS and Availability ==
#endif
...
// TARGET_OS_IOS_LIKE -- Any iOS-like platform (device or simulator): iOS, tvOS, or watchOS.

#if( !defined( TARGET_OS_IOS_LIKE ) )
	#if( TARGET_OS_IPHONE )
		#define TARGET_OS_IOS_LIKE			1
	#else
		#define TARGET_OS_IOS_LIKE			0
	#endif
#endif

// TARGET_OS_IOS_LIKE_DEVICE -- Any iOS-like platform for the device (not simulator): iOS, tvOS, or watchOS.

#if( !defined( TARGET_OS_IOS_LIKE_DEVICE ) )
	#if( TARGET_OS_IOS_LIKE && !TARGET_OS_SIMULATOR )
		#define TARGET_OS_IOS_LIKE_DEVICE		1
	#else
		#define TARGET_OS_IOS_LIKE_DEVICE		0
	#endif
#endif

// TARGET_OS_IOS_LIKE_SIMULATOR -- Any iOS-like platform for the simulator (not device): iOS, tvOS, or watchOS.

#if( !defined( TARGET_OS_IOS_LIKE_SIMULATOR ) )
	#if( TARGET_OS_IOS_LIKE && TARGET_OS_SIMULATOR )
		#define TARGET_OS_IOS_LIKE_SIMULATOR		1
	#else
		#define TARGET_OS_IOS_LIKE_SIMULATOR		0
	#endif
#endif

// TARGET_OS_MACOSX / TARGET_OS_OSX -- Mac OS X (but not iOS or the iOS Simulator).
	...
	#define TARGET_OS_OSX				TARGET_OS_MACOSX
#endif

// AvailabilityInternal.h doesn't set *_MAX_ALLOWED correctly for some platforms (tvOS and watchOS) so create 
// compatibility defines to make sure *_MAX_ALLOWED is at least a high as *_MIN_REQUIRED.
// <radar:31015180> is tracking fixing this in the system Availability headers.

#if( defined( __TV_OS_VERSION_MAX_ALLOWED ) && defined( __TV_OS_VERSION_MIN_REQUIRED ) && \
	 ( __TV_OS_VERSION_MAX_ALLOWED < __TV_OS_VERSION_MIN_REQUIRED ) )
	#define __TV_OS_VERSION_MAX_ALLOWED_COMPAT			__TV_OS_VERSION_MIN_REQUIRED
#else
	#define __TV_OS_VERSION_MAX_ALLOWED_COMPAT			__TV_OS_VERSION_MAX_ALLOWED
#endif
#if( defined( __WATCH_OS_VERSION_MAX_ALLOWED ) && defined( __WATCH_OS_VERSION_MIN_REQUIRED ) && \
	 ( __WATCH_OS_VERSION_MAX_ALLOWED < __WATCH_OS_VERSION_MIN_REQUIRED ) )
	#define __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT		__WATCH_OS_VERSION_MIN_REQUIRED
#else
	#define __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT		__WATCH_OS_VERSION_MAX_ALLOWED
#endif

// TARGET_SDK_CINAR_ALIGNED_OR_LATER
// Building for Cinar (iOS 11.2), Chipmunk (macOS 10.13.2), Coyote (tvOS 11.2), Catamaran (watchOS 4.2), or later.
// December 2017

#if( !defined( TARGET_SDK_CINAR_ALIGNED_OR_LATER ) )
	#if( ( TARGET_OS_IOS    && ( __IPHONE_OS_VERSION_MAX_ALLOWED		>= 110200 ) ) || \
		 ( TARGET_OS_MACOSX && ( __MAC_OS_X_VERSION_MAX_ALLOWED			>= 101302 ) ) || \
		 ( TARGET_OS_TV     && ( __TV_OS_VERSION_MAX_ALLOWED_COMPAT		>= 110200 ) ) || \
		 ( TARGET_OS_WATCH  && ( __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT	>= 40200 ) ) )
		#define TARGET_SDK_CINAR_ALIGNED_OR_LATER		1
	#else
		#define TARGET_SDK_CINAR_ALIGNED_OR_LATER		0
	#endif
#endif
#if( TARGET_OS_DARWIN )
	// It's 10.13 instead of 10.13.2 because of <radar:34423644>.
	#define AVAILABLE_CINAR_ALIGNED		API_AVAILABLE( ios( 11.2 ), macos( 10.13 ), tvos( 11.2 ), watchos( 4.2 ) )
#else
	#define AVAILABLE_CINAR_ALIGNED		EXPORT_GLOBAL
#endif

// TARGET_SDK_EMET_ALIGNED_OR_LATER
// Building for Emet (iOS 11.3), Elk (macOS 10.13.4), Eaton (tvOS 11.3), Emperor (watchOS 4.3), or later.
// Spring 2018

#if( !defined( TARGET_SDK_EMET_ALIGNED_OR_LATER ) )
	#if( ( TARGET_OS_IOS    && ( __IPHONE_OS_VERSION_MAX_ALLOWED		>= 110300 ) ) || \
		 ( TARGET_OS_MACOSX && ( __MAC_OS_X_VERSION_MAX_ALLOWED			>= 101304 ) ) || \
		 ( TARGET_OS_TV     && ( __TV_OS_VERSION_MAX_ALLOWED_COMPAT		>= 110300 ) ) || \
		 ( TARGET_OS_WATCH  && ( __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT	>= 40300 ) ) )
		#define TARGET_SDK_EMET_ALIGNED_OR_LATER		1
	#else
		#define TARGET_SDK_EMET_ALIGNED_OR_LATER		0
	#endif
#endif
#if( TARGET_OS_DARWIN )
	// It's 10.13 instead of 10.13.4 because of <radar:34423644>.
	#define AVAILABLE_EMET_ALIGNED		API_AVAILABLE( ios( 11.3 ), macos( 10.13 ), tvos( 11.3 ), watchos( 4.3 ) )
#else
	#define AVAILABLE_EMET_ALIGNED		EXPORT_GLOBAL
#endif

// TARGET_SDK_PEACE_ALIGNED_OR_LATER
// Building for Peace (iOS 12.0), Liberty (macOS 10.14), Hope (tvOS 12.0), Glory (watchOS 5.0), or later.
// Fall 2018

#if( !defined( TARGET_SDK_PEACE_ALIGNED_OR_LATER ) )
	#if( ( TARGET_OS_IOS    && ( __IPHONE_OS_VERSION_MAX_ALLOWED		>= 120000 ) ) || \
		 ( TARGET_OS_MACOSX && ( __MAC_OS_X_VERSION_MAX_ALLOWED			>= 101400 ) ) || \
		 ( TARGET_OS_TV     && ( __TV_OS_VERSION_MAX_ALLOWED_COMPAT		>= 120000 ) ) || \
		 ( TARGET_OS_WATCH  && ( __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT	>= 50000 ) ) )
		#define TARGET_SDK_PEACE_ALIGNED_OR_LATER		1
	#else
		#define TARGET_SDK_PEACE_ALIGNED_OR_LATER		0
	#endif
#endif
#if( TARGET_OS_DARWIN )
	#define AVAILABLE_PEACE_ALIGNED		API_AVAILABLE( ios( 12.0 ), macos( 10.14 ), tvos( 12.0 ), watchos( 5.0 ) )
#else
	#define AVAILABLE_PEACE_ALIGNED		EXPORT_GLOBAL
#endif
#if( TARGET_OS_DARWIN )
	#define IS_AVAILABLE_PEACE_ALIGNED_OR_LATER		@available( iOS 12.0, macOS 10.14, tvOS 12.0, watchOS 5.0, * )
#else
	#define IS_AVAILABLE_PEACE_ALIGNED_OR_LATER		1
#endif

// TARGET_SDK_YUKON_ALIGNED_OR_LATER
// Building for Yukon (iOS 13.0), Jazz (macOS 10.15), Yager (tvOS 13.0), Grace (watchOS 6.0), or later.
// Fall 2019

#if( defined( __IPHONE_OS_VERSION_MAX_ALLOWED ) && ( __IPHONE_OS_VERSION_MAX_ALLOWED >= 130000 ) )
	#define YUKON_MARKETING_VERSION_compat		13.0
#else
	#define YUKON_MARKETING_VERSION_compat		12.0
#endif
#if( defined( __MAC_OS_X_VERSION_MAX_ALLOWED ) && ( __MAC_OS_X_VERSION_MAX_ALLOWED >= 101500 ) )
	#define JAZZ_MARKETING_VERSION_compat		10.15
#else
	#define JAZZ_MARKETING_VERSION_compat		10.14
#endif
#if( defined( __TV_OS_VERSION_MAX_ALLOWED ) && ( __TV_OS_VERSION_MAX_ALLOWED_COMPAT >= 130000 ) )
	#define YAGER_MARKETING_VERSION_compat		13.0
#else
	#define YAGER_MARKETING_VERSION_compat		12.0
#endif
#if( defined( __WATCH_OS_VERSION_MAX_ALLOWED ) && ( __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT >= 60000 ) )
	#define GRACE_MARKETING_VERSION_compat		6.0
#else
	#define GRACE_MARKETING_VERSION_compat		5.0
#endif

#if( !defined( TARGET_SDK_YUKON_ALIGNED_OR_LATER ) )
	#if( ( TARGET_OS_IOS    && ( __IPHONE_OS_VERSION_MAX_ALLOWED		>= 130000 ) ) || \
		 ( TARGET_OS_MACOSX && ( __MAC_OS_X_VERSION_MAX_ALLOWED			>= 101500 ) ) || \
		 ( TARGET_OS_TV     && ( __TV_OS_VERSION_MAX_ALLOWED_COMPAT		>= 130000 ) ) || \
		 ( TARGET_OS_WATCH  && ( __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT	>= 60000 ) ) || \
		 ( TARGET_OS_DARWIN_REAL && ( _DNS_SD_H >= 9040000 ) ) )
		#define TARGET_SDK_YUKON_ALIGNED_OR_LATER		1
	#else
		#define TARGET_SDK_YUKON_ALIGNED_OR_LATER		0
	#endif
#endif
#if( TARGET_OS_DARWIN )
	#define AVAILABLE_YUKON_ALIGNED		API_AVAILABLE( ios( YUKON_MARKETING_VERSION_compat ), macos( JAZZ_MARKETING_VERSION_compat ), tvos( YAGER_MARKETING_VERSION_compat ), watchos( GRACE_MARKETING_VERSION_compat ) )
#else
	#define AVAILABLE_YUKON_ALIGNED		EXPORT_GLOBAL
#endif
#if( TARGET_OS_DARWIN )
	#define IS_AVAILABLE_YUKON_ALIGNED_OR_LATER		@available( iOS YUKON_MARKETING_VERSION_compat, macOS JAZZ_MARKETING_VERSION_compat, tvOS YAGER_MARKETING_VERSION_compat, watchOS GRACE_MARKETING_VERSION_compat, * )
#else
	#define IS_AVAILABLE_YUKON_ALIGNED_OR_LATER		1
#endif

// TARGET_SDK_T_TRAIN_OR_LATER
// Building for Tigris (iOS 11.0) and Topaz (tvOS 11.0) ... also Fortune (watchOS 4.0) and Lobo (macOS 10.13).
// The Apple Program Office needs to come with a better way to identify releases that all come out at about the same time.
// Since the previous 3 released all started with the same first letter and 2 of the new trains start with a T, I'm 
// calling them the T trains. Maybe they can rename those other 2 with a T to make this stuff easier.

#if( !defined( TARGET_SDK_T_TRAIN_OR_LATER ) )
	#if( ( TARGET_OS_IOS    && ( __IPHONE_OS_VERSION_MAX_ALLOWED		>= 110000 ) ) || \
		 ( TARGET_OS_MACOSX && ( __MAC_OS_X_VERSION_MAX_ALLOWED			>= 101300 ) ) || \
		 ( TARGET_OS_TV     && ( __TV_OS_VERSION_MAX_ALLOWED_COMPAT		>= 110000 ) ) || \
		 ( TARGET_OS_WATCH  && ( __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT	>= 40000 ) ) )
		#define TARGET_SDK_T_TRAIN_OR_LATER		1
	#else
		#define TARGET_SDK_T_TRAIN_OR_LATER		0
	#endif
#endif
#if( TARGET_OS_DARWIN )
	#define AVAILABLE_T_TRAINS		API_AVAILABLE( ios( 11.0 ), macos( 10.13 ), tvos( 11.0 ), watchos( 4.0 ) )
#else
	#define AVAILABLE_T_TRAINS		EXPORT_GLOBAL
#endif

// TARGET_SDK_E_TRAIN_OR_LATER
// Building for Erie (iOS 10.3), SUFujiEvolution (macOS 10.12.4), Emerald (tvOS 10.2), and Electric (watchOS 3.2).

#if( !defined( TARGET_SDK_E_TRAIN_OR_LATER ) )
	#if( ( TARGET_OS_IOS    && ( __IPHONE_OS_VERSION_MAX_ALLOWED		>= 100300 ) ) || \
		 ( TARGET_OS_MACOSX && ( __MAC_OS_X_VERSION_MAX_ALLOWED			>= 101204 ) ) || \
		 ( TARGET_OS_TV     && ( __TV_OS_VERSION_MAX_ALLOWED_COMPAT		>= 100200 ) ) || \
		 ( TARGET_OS_WATCH  && ( __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT	>= 30200 ) ) )
		#define TARGET_SDK_E_TRAIN_OR_LATER		1
	#else
		#define TARGET_SDK_E_TRAIN_OR_LATER		0
	#endif
#endif
#if( TARGET_OS_DARWIN )
	#define AVAILABLE_E_TRAINS		API_AVAILABLE( ios( 10.3 ), macos( 10.12.4 ), tvos( 10.2 ), watchos( 3.2 ) )
#else
	#define AVAILABLE_E_TRAINS		EXPORT_GLOBAL
#endif

// TARGET_SDK_C_TRAIN_OR_LATER
// Building for Corry (iOS 10.2), SUFujiCascade (macOS 10.12.2), Clementine (tvOS 10.1), and Catfish (watchOS 3.1.1).

#if( !defined( TARGET_SDK_C_TRAIN_OR_LATER ) )
	#if( ( TARGET_OS_IOS    && ( __IPHONE_OS_VERSION_MAX_ALLOWED		>= 100200 ) ) || \
		 ( TARGET_OS_MACOSX && ( __MAC_OS_X_VERSION_MAX_ALLOWED			>= 101202 ) ) || \
		 ( TARGET_OS_TV     && ( __TV_OS_VERSION_MAX_ALLOWED_COMPAT		>= 100100 ) ) || \
		 ( TARGET_OS_WATCH  && ( __WATCH_OS_VERSION_MAX_ALLOWED_COMPAT	>= 30101 ) ) )
		#define TARGET_SDK_C_TRAIN_OR_LATER		1
	#else
		#define TARGET_SDK_C_TRAIN_OR_LATER		0
	#endif
#endif
#if( TARGET_OS_DARWIN )
	#define AVAILABLE_C_TRAINS		API_AVAILABLE( ios( 10.2 ), macos( 10.12.2 ), tvos( 10.1 ), watchos( 3.1.1 ) )
#else
	#define AVAILABLE_C_TRAINS		EXPORT_GLOBAL
#endif

// AVAILABLE_ALWAYS

#if( TARGET_OS_DARWIN )
	#define AVAILABLE_ALWAYS		__attribute__( ( visibility( "default" ) ) )
#else
	#define AVAILABLE_ALWAYS		EXPORT_GLOBAL
#endif
...
#if( COMPILER_ARC )
	STATIC_INLINE id _Nullable	arc_safe_autorelease( id _Nullable x )	{ return( x ); }
	#define						arc_safe_release( x )					do {} while( 0 )
	STATIC_INLINE id _Nullable	arc_safe_retain( id _Nullable x )		{ return( x ); }
...
// CF_ENABLED: 1=CoreFoundation APIs can be used (either the real CF or CFLite). 0=CoreFoundation cannot be used.
	...
	#if( ( CFLITE_ENABLED || TARGET_OS_DARWIN || TARGET_OS_WINDOWS ) && !TARGET_KERNEL )
		#define	CF_ENABLED		1
	#else
		#define	CF_ENABLED		0
	#endif
...
// CU_HAS_MODERN_RUNTIME -- Has Objective-C 2.0 or later and OS objects like dispatch_queue_t are Objective-C objects.

#if( defined( __OBJC2__ ) && __OBJC2__ && defined( OS_OBJECT_USE_OBJC ) && OS_OBJECT_USE_OBJC )
	#define CU_HAS_MODERN_RUNTIME		1
#else
	#define CU_HAS_MODERN_RUNTIME		0
#endif
...
// TARGET_HAS_AUDIO_SESSION: 1=Platform supports CoreAudio's AudioSession APIs (and AVFoundation's AVAudioSession).
	...
	#if( TARGET_OS_IPHONE )
		#define TARGET_HAS_AUDIO_SESSION		1
	#else
		#define TARGET_HAS_AUDIO_SESSION		0
	#endif
#endif

// TARGET_HAS_BLE_L2CAP: 1=Has the CoreBluetooth's L2CAP connection API.
// $$$ Liberty and Jazz will be the same for while, but Liberty is missing certain features, such as BLE L2CAP, 
// but there isn't a way to detect this at compile time so we just have to turn it off for now. B&I should have
// bumped Jazz to 10.15, let projects update, and then bumped Liberty to 10.14, but they didn't. Once they fix
// this, TARGET_HAS_BLE_L2CAP can be re-enabled for TARGET_SDK_YUKON_ALIGNED_OR_LATER.

#if( !defined( TARGET_HAS_BLE_L2CAP ) )
	#if( TARGET_OS_IPHONE )
		#define TARGET_HAS_BLE_L2CAP		1
		#define IS_AVAILABLE_BLE_L2CAP		@available( iOS 11.3, macOS JAZZ_MARKETING_VERSION_compat, tvOS 11.3, watchOS 4.3, * )
	#else
		#define TARGET_HAS_BLE_L2CAP		0
	#endif
#endif

// TARGET_HAS_CALLKIT: CallKit API is available.

#if( !defined( TARGET_HAS_CALLKIT ) )
	#if( TARGET_OS_IOS || TARGET_OS_MACOSX || TARGET_OS_WATCH )
		#define TARGET_HAS_CALLKIT		1
	#else
		#define TARGET_HAS_CALLKIT		0
	#endif
#endif

// TARGET_HAS_CBScalablePipeRegisterEndpointOptions -- Has CBScalablePipeManager "registerEndpoint:type:priority:options:".

#if( !defined( TARGET_HAS_CBScalablePipeRegisterEndpointOptions ) )
	#if( TARGET_SDK_PEACE_ALIGNED_OR_LATER )
		#define TARGET_HAS_CBScalablePipeRegisterEndpointOptions		1
		#define IS_AVAILABLE_CBScalablePipeRegisterEndpointOptions		IS_AVAILABLE_PEACE_ALIGNED_OR_LATER
	#else
		#define TARGET_HAS_CBScalablePipeRegisterEndpointOptions		0
	#endif
#endif
...
// TARGET_HAS_CORE_WLAN: CoreWLAN API is available.

#if( !defined( TARGET_HAS_CORE_WLAN ) )
	#if( TARGET_OS_MACOSX )
		#define TARGET_HAS_CORE_WLAN		1
	#else
		#define TARGET_HAS_CORE_WLAN		0
	#endif
#endif

// TARGET_HAS_EXTERNAL_ACCESSORY_API: 1=Has the MFi ExternalAccessory framework.

#if( !defined( TARGET_HAS_EXTERNAL_ACCESSORY_API ) )
	#if( TARGET_OS_IPHONE )
		#define TARGET_HAS_EXTERNAL_ACCESSORY_API		1
	#else
		#define TARGET_HAS_EXTERNAL_ACCESSORY_API		0
	#endif
#endif

// TARGET_HAS_FIND_MY_FRIENDS: 1=Has the FMF framework.

#if( !defined( TARGET_HAS_FIND_MY_FRIENDS ) )
	#if( TARGET_OS_IOS || TARGET_OS_MACOSX )
		#define TARGET_HAS_FIND_MY_FRIENDS		1
	#else
		#define TARGET_HAS_FIND_MY_FRIENDS		0
	#endif
#endif

// TARGET_HAS_HOMEKIT: 1=Has HomeKit framework.

#if( !defined( TARGET_HAS_HOMEKIT ) )
	#if( TARGET_OS_IPHONE )
		#define TARGET_HAS_HOMEKIT		1
	#else
		#define TARGET_HAS_HOMEKIT		0
	#endif
#endif
...
// TARGET_HAS_MOBILE_DEVICE: MobileDevice framework is available.

#if( !defined( TARGET_HAS_MOBILE_DEVICE ) )
	#if( TARGET_OS_MACOSX && CU_HAS_MODERN_RUNTIME )
		#define TARGET_HAS_MOBILE_DEVICE		1
	#else
		#define TARGET_HAS_MOBILE_DEVICE		0
	#endif
#endif

// TARGET_HAS_MOBILE_BLUETOOTH: MobileBluetooth framework is available.

#if( !defined( TARGET_HAS_MOBILE_BLUETOOTH ) )
	#if( TARGET_OS_IPHONE && TARGET_OS_DARWIN_REAL )
		#define TARGET_HAS_MOBILE_BLUETOOTH		1
	#else
		#define TARGET_HAS_MOBILE_BLUETOOTH		0
	#endif
#endif

// TARGET_HAS_MOBILE_GESTALT: MobileGestalt API is available.

#if( !defined( TARGET_HAS_MOBILE_GESTALT ) )
	#if( TARGET_OS_IPHONE && TARGET_OS_DARWIN_REAL )
		#define TARGET_HAS_MOBILE_GESTALT		1
	#else
		#define TARGET_HAS_MOBILE_GESTALT		0
	#endif
#endif

// TARGET_HAS_MOBILE_WIFI: MobileWiFi API is available.

#if( !defined( TARGET_HAS_MOBILE_WIFI ) )
	#if( TARGET_OS_IOS_LIKE_DEVICE )
		#define TARGET_HAS_MOBILE_WIFI		1
	#else
		#define TARGET_HAS_MOBILE_WIFI		0
	#endif
#endif

// TARGET_HAS_NFC: NearField API is available. NFC hardware may not be available.

#if( !defined( TARGET_HAS_NFC ) )
	#if( TARGET_OS_IOS_LIKE_DEVICE && !TARGET_OS_TV )
		#define TARGET_HAS_NFC		1
	#else
		#define TARGET_HAS_NFC		0
	#endif
#endif

// TARGET_HAS_POST_SOCKETS: Post-Sockets API (i.e. nw_connection, etc.) is available.

#if( !defined( TARGET_HAS_POST_SOCKETS ) )
	#if( TARGET_SDK_YUKON_ALIGNED_OR_LATER )
		#define TARGET_HAS_POST_SOCKETS		1
	#else
		#define TARGET_HAS_POST_SOCKETS		0
	#endif
#endif
#define IS_AVAILABLE_POST_SOCKETS_API		IS_AVAILABLE_YUKON_ALIGNED_OR_LATER

// TARGET_HAS_SecCertificateCopyKey -- Has SecCertificateCopyKey API.

#if( !defined( TARGET_HAS_SecCertificateCopyKey ) )
	#if( TARGET_SDK_YUKON_ALIGNED_OR_LATER )
		#define TARGET_HAS_SecCertificateCopyKey		1
		#define IS_AVAILABLE_SecCertificateCopyKey		IS_AVAILABLE_YUKON_ALIGNED_OR_LATER
	#else
		#define TARGET_HAS_SecCertificateCopyKey		0
	#endif
#endif
...
// TARGET_HAS_SPRINGBOARD: 1=Has SpringBoard.

#if( !defined( TARGET_HAS_SPRINGBOARD ) )
	#if( TARGET_OS_IOS )
		#define TARGET_HAS_SPRINGBOARD		1
	#else
		#define TARGET_HAS_SPRINGBOARD		0
	#endif
#endif

// TARGET_HAS_WIFI_AWARE_API: 1=Has the WiFiAware API (WiFiIPeerToPeer framework).

#if( !defined( TARGET_HAS_WIFI_AWARE_API ) )
	#if( __has_include( <WiFiPeerToPeer/WiFiAware.h> ) )
		#define TARGET_HAS_WIFI_AWARE_API		1
	#else
		#define TARGET_HAS_WIFI_AWARE_API		0
	#endif
#endif
...
#if( !defined( TARGET_HAS_CORECRYPTO_CURVE25519 ) )
	#if( APPLE_INTERNAL && TARGET_OS_DARWIN )
		#define TARGET_HAS_CORECRYPTO_CURVE25519		1
	#else
		#define TARGET_HAS_CORECRYPTO_CURVE25519		0
	#endif
#endif
#if( TARGET_HAS_CORECRYPTO_CURVE25519 )
	#define curve25519( RESULT, SK, BASE )		cccurve25519( (RESULT), (SK), (BASE) )
#else
...
// Ed25519
...
#if( !defined( TARGET_HAS_CORECRYPTO_ED25519 ) )
	#if( APPLE_INTERNAL && TARGET_OS_DARWIN )
		#define TARGET_HAS_CORECRYPTO_ED25519		1
	#else
		#define TARGET_HAS_CORECRYPTO_ED25519		0
	#endif
#endif
#if( TARGET_HAS_CORECRYPTO_ED25519 )
	#ifdef __cplusplus
	extern "C" {
	#endif
	
	AVAILABLE_ALWAYS
	extern void	cced25519_make_key_pair_compat( uint8_t outPK[ 32 ], uint8_t outSK[ 32 ] );
	AVAILABLE_ALWAYS
	extern void	cced25519_sign_compat( uint8_t outSig[ 64 ], const void * _Nullable inMsg, size_t inLen, 
		const uint8_t inPK[ 32 ], const uint8_t inSK[ 32 ] );
	AVAILABLE_ALWAYS
	extern int	cced25519_verify_compat( const void * _Nullable inMsg, size_t inLen, const uint8_t inSig[ 64 ], 
	...
	#define Ed25519_make_key_pair( PK, SK )				cced25519_make_key_pair_compat( (PK), (SK) )
	#define Ed25519_sign( SIG, MSG, LEN, PK, SK )		cced25519_sign_compat( (SIG), (MSG), (LEN), (PK), (SK) )
	#define Ed25519_verify( MSG, LEN, SIG, PK )			cced25519_verify_compat( (MSG), (LEN), (SIG), (PK) )
#else
...
#if( !defined( TARGET_HAS_CORECRYPTO_HKDF ) )
	#if( APPLE_INTERNAL && TARGET_OS_DARWIN )
		#define TARGET_HAS_CORECRYPTO_HKDF		1
	#else
		#define TARGET_HAS_CORECRYPTO_HKDF		0
	#endif
#endif
#if( TARGET_HAS_CORECRYPTO_HKDF )
	#define HKDF_SHA512( INPUT_PTR, INPUT_LEN, SALT_PTR, SALT_LEN, INFO_PTR, INFO_LEN, OUTPUT_LEN, OUTPUT_PTR ) \
		cchkdf( ccsha512_di(), (INPUT_LEN), (INPUT_PTR), (SALT_LEN), (SALT_PTR), (INFO_LEN), (INFO_PTR), \
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Performs a compile-time check of something such as the size of an int.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Perform a compile-time check, suitable for placement in code, of something such as the size of an int.
...
#if( TARGET_OS_EFI && !defined( __GNUC__ ) )
	#define CHAR_BIT				8
...
#if( TARGET_LANGUAGE_C_LIKE )
	#if( !defined( _BSD_SSIZE_T_DEFINED_ ) && !defined( _SSIZE_T ) && !defined( _SSIZE_T_DEFINED ) && \
		 !TARGET_OS_VXWORKS && !TARGET_OS_LINUX && !TARGET_OS_NETBSD )
...
#if( ( TARGET_OS_EFI && !defined( __GNUC__ ) ) || \
	 ( TARGET_PLATFORM_WICED && ( !defined( _TIMEVAL_DEFINED ) || !_TIMEVAL_DEFINED ) ) )
	struct timeval
	{
		int32_t		tv_sec;
		int32_t		tv_usec;
...
STATIC_INLINE Float32	ReadBigFloat32( const void * _Nonnull p )			{ Value32 v32; v32.u32 = ReadBig32( p ); return( v32.f32 ); }
STATIC_INLINE Float64	ReadBigFloat64( const void * _Nonnull p )			{ Value64 v64; v64.u64 = ReadBig64( p ); return( v64.f64 ); }
STATIC_INLINE Float32	ReadLittleFloat32( const void * _Nonnull p )		{ Value32 v32; v32.u32 = ReadLittle32( p ); return( v32.f32 ); }
STATIC_INLINE Float64	ReadLittleFloat64( const void * _Nonnull p )		{ Value64 v64; v64.u64 = ReadLittle64( p ); return( v64.f64 ); }

STATIC_INLINE void		WriteBigFloat32( void * _Nonnull p, Float32 x )		{ Value32 v32; v32.f32 = x; WriteBig32( p, v32.u32 ); }
STATIC_INLINE void		WriteBigFloat64( void * _Nonnull p, Float64 x )		{ Value64 v64; v64.f64 = x; WriteBig64( p, v64.u64 ); }
STATIC_INLINE void		WriteLittleFloat32( void * _Nonnull p, Float32 x )	{ Value32 v32; v32.f32 = x; WriteLittle32( p, v32.u32 ); }
STATIC_INLINE void		WriteLittleFloat64( void * _Nonnull p, Float64 x )	{ Value64 v64; v64.f64 = x; WriteLittle64( p, v64.u64 ); }
...
// Macros to allow the same code to work on Windows and other sockets API-compatible platforms.
	...
	#define	set_errno_compat( X )				do { errno = (X); } while( 0 )
		...
		#define ENOMEM					kNoMemoryErr
		#define EINVAL					kParamErr
		#define ENAMETOOLONG			kNameErr
	#endif
...
#elif( TARGET_OS_EFI )
	#define iovec_t			EFI_UDP4_FRAGMENT_DATA
	#define iov_len			FragmentLength
	#define iov_base		FragmentBuffer
...
// EFI doesn't have stdarg.h or string.h, but it does have some equivalents so map them to the standard names.
		...
		#define	va_start( LIST, ARG )		VA_START( LIST, ARG )
	...
	#define calloc( COUNT, SIZE )			EfiLibAllocateZeroPool( (COUNT) * (SIZE) )
	...
	#define isspace( X ) 				    ( ( (X) == ' ' ) || ( ( (X) >= 9 ) && ( (X) <= 13 ) ) )
	#define islower( X )					( ( (X) >= 'a' ) && ( (X) <= 'z' ) )
	#define isupper( X )					( ( (X) >= 'A' ) && ( (X) <= 'Z' ) )
	#define tolower( X )					( isupper( X ) ? ( (X) + 32 ) : (X) )
	#define toupper( X )					( islower( X ) ? ( (X) - 32 ) : (X) )
	
	#define	memcmp( A, B, SIZE )			EfiCompareMem( (void *)(A), (void *)(B), SIZE )
	#define	memcpy( DST, SRC, SIZE )		EfiCopyMem( DST, (void *)(SRC), SIZE )
	#define	memmove( DST, SRC, SIZE )		EfiCopyMem( DST, (void *)(SRC), SIZE )
	#define	memset( DST, VAL, SIZE )		EfiSetMem( DST, SIZE, (UINT8)( VAL ) )
	...
	char *	strcpy( char *inDst, const char *inSrc );
	#define strlen( STR )					EfiAsciiStrLen( (CHAR8 *)( STR ) )
	#define snprintf						SNPrintF
	#define	strcasecmp( S1, S2 )			stricmp( (S1), (S2) )
	#define	strncasecmp( S1, S2, SIZE )		strnicmp( (S1), (S2), (SIZE) )
	int		strcmp( const char *inS1, const char *inS2 );
	int		strncmp( const char *inS1, const char *inS2, size_t inMax );
...
// versions with an _s suffix (supposedly more secure). So just map the standard functions to those.
	...
	#if( TARGET_VISUAL_STUDIO_2005_OR_LATER && !defined( snprintf ) )
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macro for Linux-style branch prediction.
...
// CURVE25519_HEADER -- Header file to include for Curve25519.
	...
	#endif
#elif( TARGET_HAS_CORECRYPTO_CURVE25519 )
	#define CURVE25519_HEADER		<corecrypto/ccec25519.h>
#elif( FRAMEWORK_STYLE_INCLUDES )
...
// ED25519_HEADER -- Header file to include for Ed25519.
	...
	#endif
#elif( TARGET_HAS_CORECRYPTO_ED25519 )
	#define ED25519_HEADER			<corecrypto/ccec25519.h>
#elif( FRAMEWORK_STYLE_INCLUDES )
...
// LIBNETCORE_HEADER -- Header file to include for libnetcore (nw_connection, etc.).

#if( !defined( LIBNETCORE_HEADER ) )
	#if( __has_include( <nw/private.h> ) )
		#define LIBNETCORE_HEADER		<nw/private.h>
	#elif( __has_include( <network/private.h> ) )
		#define LIBNETCORE_HEADER		<network/private.h>
	#else
		#define LIBNETCORE_HEADER		"CommonServices.h"
	#endif
#endif
...
// sockaddr_in
	...
	struct sockaddr_in
	{
		struct sockaddr		sa_base;
		uint16_t			sin_port;
		struct in_addr		sin_addr;
		char				sin_zero[ 8 ];
	};

	#define sin_family		sa_base.sa_family
#endif
...
// sockaddr_in6
	...
	#define sin6_family		sa_base.sa_family
#endif
...
// sockaddr_dl
	...
	#define sdl_family		sa_base.sa_family
#endif
...
// Determines if a sockaddr is an unspecified address (i.e. 0.0.0.0 or ::).

#if( defined( AF_INET6 ) )
	#define	SockAddrIsUnspecified( X ) \
		( ( ( (const struct sockaddr *)(X) )->sa_family == AF_INET ) \
		  ? ( ( (const struct sockaddr_in *)(X) )->sin_addr.s_addr == 0 ) \
		  : ( ( (const struct sockaddr *)(X) )->sa_family == AF_INET6 ) \
			? IN6_IS_ADDR_UNSPECIFIED( &( (const struct sockaddr_in6 *)(X) )->sin6_addr ) \
			: 0 )
#else
	#define	SockAddrIsUnspecified( X ) \
		( ( ( (const struct sockaddr *)(X) )->sa_family == AF_INET ) \
...
#if( !APPLE_EXTERNAL )
#define kDNSServiceFlagsIncludeAWDL_compat			0x100000U	// Added in version 380 of dns_sd.h.
#endif
...
#define kDNSServiceFlagsThresholdReached_compat		0x2000000U	// Added in version 504 of dns_sd.h
	...
	#if( CU_BUILDING_FOR_TEST_ENVIRONMENT )
		#define kSecAttrAccessibleAlways_compat		CFSTR( "dk" )
	#else
		#define kSecAttrAccessibleAlways_compat		kSecAttrAccessibleAlwaysPrivate
	#endif
#endif
...
// HAS_CF_DISTRIBUTED_NOTIFICATIONS
	...
	#if( ( TARGET_OS_DARWIN && !TARGET_OS_IPHONE ) || TARGET_OS_WINDOWS )
...
#if( HAS_CF_DISTRIBUTED_NOTIFICATIONS && TARGET_OS_IPHONE )
	#define kCFNotificationPostToAllSessions			0 // No sessions on iOS so use 0 so things can still compile.
#endif
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns a random number that's >= MIN and <= MAX (i.e. it's inclusive). MIN must be <= MAX.
*/
#define RandomRange( MIN, MAX )		( (MIN) + ( Random32() % ( ( (MAX) - (MIN) ) + 1 ) ) )
#define RandomRangeF( MIN, MAX )	( ( ( Random32() / 4294967295.0 ) * ( (MAX) - (MIN) ) ) + (MIN) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns a cryptographically strong 32-bit random number.
*/
#if( TARGET_OS_BSD )
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Some compilers allow you to mark printf-style functions so their format string and argument list 
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macro to mark a function as exported to other code within the same package.
*/
#if( TARGET_OS_DARWIN_KERNEL )
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macro to mark a function as exported outside of the package.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macro to mark a data symbol as an DLL import.
	
	To import, declare it like this
	
	IMPORT_GLOBAL int		gMyGlobal;
*/
#if( TARGET_OS_WINDOWS && !( defined( BUILDING_COREUTILS ) && BUILDING_COREUTILS ) )
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Wrappers for the ctype.h macros make them safe when used with signed characters.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		CoreFoundation object creation/subclassing to make it easier to create CFType subclasses.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		A meta-value to pass to supported routines to indicate the size should be calculated with strlen.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Number of bytes from the beginning of the type to the specified field.
*/
#if( !defined( offsetof ) )
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Determines the number of bytes from the start of a structure until the end of a field.
*/
#define	endof_field( TYPE, FIELD )		( offsetof( TYPE, FIELD ) + sizeof_field( TYPE, FIELD ) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Determines the size of an array element.
*/
#define	sizeof_element( X )		sizeof( X[ 0 ] )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Determines the size of a field of a type.
*/
#define	sizeof_field( TYPE, FIELD )		sizeof( ( ( (TYPE *) 0 )->FIELD ) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Determines the size of a constant C string, excluding the null terminator.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns true if the value differs from 0 by less than the rounding error for a floating point number.
*/
#define FloatEquivalentToZero( X )		( fabs( (X) - 0 ) < FLT_EPSILON )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns true if the value differs from 1 by less than the rounding error for a floating point number.
*/
#define FloatEquivalentToOne( X )		( fabs( (X) - 1 ) < FLT_EPSILON )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns true if the two value values differ by less than the rounding error for a floating point number.
	@details	See also <https://randomascii.wordpress.com/2012/02/25/comparing-floating-point-numbers-2012-edition/>.
*/
#define FloatsEquivalent( X, Y )		( fabs( (X) - (Y) ) < FLT_EPSILON )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Rounds VALUE down to the nearest multiple of MULTIPLE.
*/
#define	RoundDown( VALUE, MULTIPLE )	( ( (VALUE) / (MULTIPLE) ) * (MULTIPLE) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Rounds VALUE up to the nearest multiple of MULTIPLE.
*/
#define	RoundUp( VALUE, MULTIPLE )		( ( ( (VALUE) + ( (MULTIPLE) - 1 ) ) / (MULTIPLE) ) * (MULTIPLE) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Rounds a value to a specific precision.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns non-zero if X is aligned to a Y byte boundary and 0 if not. Y must be a power of 2.
*/
#define	IsAligned( X, Y )		( ( (X) & ( (Y) - 1 ) ) == 0 )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns non-zero if FIELD of type TYPE is aligned to a Y byte boundary and 0 if not. Y must be a power of 2.
*/
#define	IsFieldAligned( X, TYPE, FIELD, Y )		IsAligned( ( (uintptr_t)(X) ) + offsetof( TYPE, FIELD ), (Y) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns true if the two ptr/len pairs overlap each other.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns non-zero if PTR is aligned to a Y byte boundary and 0 if not. Y must be a power of 2.
*/
#define	IsPtrAligned( PTR, Y )		( ( ( (uintptr_t)(PTR) ) & ( (Y) - 1 ) ) == 0 )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Aligns X down to a Y byte boundary. Y must be a power of 2.
*/
#define	AlignDown( X, Y )		( (X) & ~( (Y) - 1 ) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Aligns X up to a Y byte boundary. Y must be a power of 2.
*/
#define	AlignUp( X, Y )		( ( (X) + ( (Y) - 1 ) ) & ~( (Y) - 1 ) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Specifies a buffer of a specific number of bytes that is aligned to the strictest C alignment.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	Logical and arithmetic shifts on signed values.
	
	These work around undefined behavior in C for shifts of signed values.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Rotates X COUNT bits to the left or right.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Divides a 64-bit value by a 32-bit divisor and fills in the remainder.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns non-zero if the value is odd and 0 if it is even.
*/
#define	IsOdd( X )			( ( (X) & 1 ) != 0 )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns non-zero if the value is even and 0 if it is odd.
*/
#define	IsEven( X )			( ( (X) & 1 ) == 0 )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns non-zero if the value is a power of 2 and 0 if it is not. 0 and 1 are not considered powers of 2.
*/
#define	IsPowerOf2( X )		( ( (X) > 1 ) && ( ( (X) & ( (X) - 1 ) ) == 0 ) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns the minimum number of power-of-2 bytes needed to hold a specific value.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns non-zero if X is a multiple of Y.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns the minimum value of a, b, and c.
*/
#if( !defined( Min3 ) )
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Clamps a value to no less than "a" and no greater than "b".
*/
#define Clamp( x, a, b )		Max( (a), Min( (b), (x) ) )
#define CUClamp( x, a, b )		Max( (a), Min( (b), (x) ) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Increments VAR and if it wraps to 0, set VAR to WRAP.
*/
#define	increment_wrap( VAR, WRAP )		do { ++(VAR); if( (VAR) == 0 ) { (VAR) = (WRAP); } } while( 0 )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Increments VAR unless doing so would cause it to exceed MAX.
*/
#define	increment_saturate( VAR, MAX )		do { if( (VAR) < (MAX) ) { ++(VAR); } } while( 0 )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Adds VALUE to VAR. If the result would go over MAX, VAR is capped to MAX.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns the median (middle) value given 3 values.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns the median (middle) value given 5 values.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Appends a decimal string to a buffer.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macros for working with bit arrays.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Inserts BITS (both 0 and 1 bits) into X, controlled by MASK and SHIFT, and returns the result.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Extracts bits from X, controlled by MASK and SHIFT, and returns the result.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Sets bits if the test is non-zero or clears bits if the test is zero.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Stringify's an expression.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		These take a pointer and if what it points to is valid, it gets rid of it and marks it invalid.
*/
...
// A variant of dispatch_forget to release and NULL variables marked as _Nonnull so they can be NULL'd out from dealloc.
#if( defined( OS_OBJECT_USE_OBJC ) && OS_OBJECT_USE_OBJC )
	#define	dispatch_forget_final( X ) \
	do \
	{ \
		if( *(X) ) \
		{ \
			arc_safe_dispatch_release( *(X) ); \
			*( (dispatch_object_t __strong * _Nullable)(X) ) = (dispatch_object_t){ NULL }; \
		} \
		\
	}	while( 0 )
#else
	...
	{ \
		...
		{ \
			arc_safe_dispatch_release( *(X) ); \
			*( (dispatch_object_t * _Nullable)(X) ) = (dispatch_object_t){ NULL }; \
		} \
		\
	}	while( 0 )
#endif
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		These retain/copy/etc the new thing and release/free/etc the old thing (if the old thing is valid).
*/
#define ReplaceBlock( BLOCK_PTR, NEW_BLOCK ) \
	do \
	{ \
		__typeof__( (NEW_BLOCK) )		ReplaceBlock_TempBlock = (NEW_BLOCK); \
		\
...
#define ReplaceDispatchQueue( QUEUE_PTR, NEW_QUEUE ) \
	...
	#define ReplaceObjectiveCObject( OBJECT_PTR, NEW_OBJECT ) \
		do \
		{ \
			__typeof__( (OBJECT_PTR) )		ReplaceObjectiveCObject_objectPtr = (OBJECT_PTR); \
			__typeof__( *(OBJECT_PTR) )		ReplaceObjectiveCObject_oldObject = *ReplaceObjectiveCObject_objectPtr; \
			__typeof__( (NEW_OBJECT) )		ReplaceObjectiveCObject_newObject = (NEW_OBJECT); \
			\
			[ReplaceObjectiveCObject_newObject retain]; \
			*ReplaceObjectiveCObject_objectPtr = ReplaceObjectiveCObject_newObject; \
			[ReplaceObjectiveCObject_oldObject release]; \
			\
		}	while( 0 )
#endif
	
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Returns non-zero if two ptr/len pairs are equal and 0 otherwise.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Declares/defines a qsort-compatible sort function for numeric types.
...
#define DECLARE_QSORT_NUMERIC_COMPARATOR( NAME )	int	NAME( const void * _Nonnull a, const void * _Nonnull b )

#define DEFINE_QSORT_NUMERIC_COMPARATOR( TYPE, NAME ) \
	int	NAME( const void * _Nonnull a, const void * _Nonnull b ) \
	{ \
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Performs a compile-time check for a feature flag and fails to compile if the feature is not defined.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Fixed-point math macros to work with Q32.32 numbers. <http://en.wikipedia.org/wiki/Q_(number_format)>.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Modular math/Serial Number Arithmetic per RFC 1982). See <http://en.wikipedia.org/wiki/Serial_number_arithmetic>.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Standardized boolean type. Built-in with C99 and C++, but emulated elsewhere.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Mac-style Boolean type. Emulated on non-Mac platforms.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Defines whether long long (or its equivalent) is natively supported or requires special libraries.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		OSStatus and error codes.
*/
#if( TARGET_LANGUAGE_C_LIKE )
	#if( !TARGET_OS_DARWIN )
...
#define kAccessErr					-6776	//! Access not allowed or requires additional authentication.

#define kGenericErrorEnd			-6779	//! Last generic error code (inclusive)

// Additional CoreUtils error codes are in the range -71168 to -70913.

#define kCoreUtilsErrorBase			-71168
#define kCoreUtilsErrorEnd			-70913

#define kMissingEntitlementErr		-71168 // Process is missing a required entitlement.
#define kMissingServiceTypeErr		-71167 // Service type required, but not specified.
#define kMissingPeerErr				-71166 // Peer not specified.
#define kUnsupportedServiceTypeErr	-71165 // Service type not supported for this use case.
#define kRejectedCertificateErr		-71164 // Certificate is revoked, blacklisted, etc.
#define kHomeKitSyncTimeoutErr		-71163 // Timed out waiting for HomeKit to sync.
#define kiCloudAuthTimeoutErr		-71162 // Timed out performing iCloud Companion/Proxy auth.
#define kiTunesAuthTimeoutErr		-71161 // Timed out performing iTunes Companion/Proxy auth.
#define kHeartbeatTimeoutErr		-71160 // Timed out waiting for a heartbeat request/response.
#define kWiFiTimeoutErr				-71159 // Timed out waiting for WiFi to join/become reachable.

// OSStatus namespace for mapping low-numbered errors to OSStatus.
// Allocated range 1: 300000-300999.
// Allocated range 2: 310000-311999.
// Allocated range 3: 312000-312999.
// Allocated range 4: 320000-324999.
//
// 300000-300099 HTTP/2
// 300100-300119 CoreBluetooth CBError
// 300120-300139 CoreBluetooth CBATTError
// 300140-300179 CFNetwork
// 300180-300189 SpringBoard SBSRemoteAlert
// 300190-300199 Unused
// 300200-300299 HomeKit
// 300300-300399 POSIX Signals
// 300400-300499 IDSError
// 300500-300599 IDSLocalPairing
// 300600-300699 IDSAccountRegistration
// 300700-300799 IDSResponse
// 300800-300899 IDSMisc
// 300900-300939 WirelessProximity WPError
// 300940-300959 AppleKeyStore
// 300960-300999 TouchRemote
// 301000-301999 User-reported errors.
// 310000-311999 MobileBluetooth
// 312000-312199 CoreServices (-10 to -115)
// 312200-312249 Accounts
// 312250-312299 AppleIDAuthSupport
// 312300-312349 WiFiAware errors
// 312350-312399 WiFiAware reasons
// 312400-312599 StoreServices
// 312600-312999 Unused
// 320000-324999 Siri

// Accounts

#define kAccountsErrorBase					312200 // 312200-312249
#define AccountsErrorToOSStatus( X )		( (OSStatus)( kAccountsErrorBase + (X) ) )

// AppleIDAuthSupport

#define kAppleIDAuthSupportErrorBase				312250 // 312250-312299
#define AppleIDAuthSupportErrorToOSStatus( X )		( (OSStatus)( kAppleIDAuthSupportErrorBase + (X) ) )

// AppleKeyStore

#define kAppleKeyStoreErrorBase					300940 // 300940-300959
#define AppleKeyStoreErrorToOSStatus( X )		( (OSStatus)( kAppleKeyStoreErrorBase - (X) ) )

// CoreBluetooth

#define kCBErrorBase					300100 // 300100-300119
#define CBErrorToOSStatus( X )			( (OSStatus)( kCBErrorBase + (X) ) )

#define kCBATTErrorBase					300120 // 300120-300139
#define CBATTErrorToOSStatus( X )		( (OSStatus)( kCBATTErrorBase + (X) ) )

// CoreServices

#define kCoreServicesErrorBase				312000 // 312000-312200
#define CoreServicesErrorToOSStatus( X )	( (OSStatus)( kCoreServicesErrorBase + 115 + (X) ) )

// CFNetwork

#define kCFNetworkErrorBase				300140 // 300140-300179
...
#define kIDSErrorBase									300400 // 300400-300499
#define IDSErrorToOSStatus( X )							( (OSStatus)( kIDSErrorBase + (X) ) )

#define kIDSLocalPairingErrorBase						300500 // 300500-300599
#define IDSLocalPairingErrorToOSStatus( X )				( (OSStatus)( kIDSLocalPairingErrorBase + (X) ) )

#define kIDSAccountRegistrationErrorBase				300600 // 300600-300699
#define IDSAccountRegistrationErrorToOSStatus( X )		( (OSStatus)( kIDSAccountRegistrationErrorBase + ( (X) - 100 ) ) )

#define kIDSResponseErrorBase							300700 // 300700-300799
#define IDSResponseErrorToOSStatus( X )					( (OSStatus)( kIDSResponseErrorBase + (X) ) )

#define kIDSMiscErrorBase								300800 // 300800-300899
#define IDSMiscErrorToOSStatus( X )						( (OSStatus)( kIDSMiscErrorBase + (X) ) )

// MobileBluetooth

#define kMobileBluetoothErrorBase						310000 // 310000-311999
#define MobileBluetoothToOSStatus( X )					( (OSStatus)( kMobileBluetoothErrorBase + (X) ) )

// Siri (Assistant framework)

#define kSiriErrorBase									320000 // 320000-324999
#define SiriToOSStatus( X )								( (OSStatus)( kSiriErrorBase + (X) ) )

// SpringBoard

#define kSBSRemoteAlertHandleInvalidationErrorBase				300180 // 300180-300189
#define SBSRemoteAlertHandleInvalidationErrorToOSStatus( X )	( (OSStatus)( kSBSRemoteAlertHandleInvalidationErrorBase + (X) ) )

// StoreServices

#define kStoreServicesErrorBase							312400 // 312400-312599
#define StoreServicesToOSStatus( X )					( (OSStatus)( kStoreServicesErrorBase + (X) ) )

// TouchRemote

#define kTouchRemoteErrorBase							300960 // 300960-300999
#define TouchRemoteErrorToOSStatus( X )					( (OSStatus)( kTouchRemoteErrorBase + (X) ) )

// UserError -- Errors intended to be reported to a user.

#define kUserErrorDomain								"UserErrorDomain"
#define UserErrorDomain									@"UserErrorDomain"
#define IsUserErrorRange( X )							( ( (X) >= kUserErrorMin ) && ( (X) <= kUserErrorMax ) )
#define NSErrorWithUserError( X )						( [NSError errorWithDomain:@kUserErrorDomain code:(X) userInfo:nil] )
#define NSErrorWithUserErrorEx( X, INFO )				( [NSError errorWithDomain:@kUserErrorDomain code:(X) userInfo:(INFO)] )

#define kUserErrorMin									301000
#define kUserErrorMax									301999

#define kUserErrorWiFiDisabled							301000 // User needs to turn on WiFi.
#define kUserErrorPasscodeRequired						301001 // User needs to set a passcode on their device.
#define kUserErrorBluetoothDisabled						301002 // User needs to turn on Bluetooth.
#define kUserErrorDeviceActivation						301003 // Device activation or device not activated failed.
#define kUserErrorWiFiNetworkUnsupported				301004 // WiFi network can't be used (e.g. enterprise).
#define kUserErrorNoiCloudAccount						301005 // No iCloud account.
#define kUserErrorNoiCloudHSA2							301006 // No iCloud HSA2 account.
#define kUserErrorNoiTunesAccount						301007 // No iTunes account.
#define kUserErrorNoAppleMusicSubscription				301008 // No AppleMusic subscription
#define kUserErrorNoInternet							301009 // No connection to the Internet.
#define kUserErrorHomeKitNotEnabled						301010 // HomeKit isn't enabled.
#define kUserErrorNoiCloudKeychain						301011 // iCloud Keychain syncing isn't enabled.
#define kUserErrorNotPaired								301012 // Not paired with peer.
#define kUserErrorOldMobileDevice						301013 // Old version of MobileDevice framework installed.
#define kUserErroriCloudAuthenticationFailed			301014 // iCloud/iTunes authentication failed.
#define kUserErroriCloudAccountMismatch					301015 // iCloud accounts aren't the same between devices.
#define kUserErroriCloudVerifyFailed					301016 // Failed to verify iCloud account.
#define kUserErrorNoiCloudHSA2OriTunes					301017 // No iCloud HSA2 account or iTunes sign in.
#define kUserErrorNonOwnerHome							301018 // User isn't the owner of the home. Need to contact owner.
#define kUserErrorHomeAppNotInstalled					301019 // Home app isn't installed.
#define kUserErrorAppleWiFiNotAllowed					301020 // AppleWiFi not allowed.
#define kUserErrorWiFiJoinFailed						301021 // Join a WiFi network failed. Details in underlying error.
#define kUserErrorLocationServicesRequired				301022 // Location Services is needed to continue (ask user).
#define kUserErrorSiriRequired							301023 // Siri needs to be enabled to continue (ask user).
#define kUserErrorHomeKitResetRequired					301024 // HomeKit requires a reset (ask user).
#define kUserErroriCloudTermsRequired					301025 // User required to agree to iCloud terms and conditions.
#define kUserErroriTunesTermsRequired					301026 // User required to agree to iTunes terms and conditions.

// WiFiAware errors

#define kWiFiAwareErrorBase								312300 // 312300-312349
#define WiFiAwareErrorToOSStatus( X )					( (OSStatus)( kWiFiAwareErrorBase + (X) ) )

// WiFiAware reasons

#define kWiFiAwareReasonBase							312350 // 312350-312399
#define WiFiAwareReasonToOSStatus( X )					( (OSStatus)( kWiFiAwareReasonBase + (X) ) )

// WirelessProximity WPError

#define kWPErrorBase									300900 // 300900-300940
#define WPErrorToOSStatus( X )							( (OSStatus)( kWPErrorBase + (X) ) )
...
#define CUErrorKeyDescription		@"cuErrorDesc"
#define CUErrorKeyMessage			@"cuErrorMsg"
...
#define kLogLevelChatty				10			//! Similar to LOG_DEBUG / OS_LOG_TYPE_DEBUG
#define kLogLevelVerbose			20
#define kLogLevelTrace 				30
#define kLogLevelInfo 				40			//! Similar to LOG_INFO
#define kLogLevelNotice				50			//! Similar to LOG_NOTICE / OS_LOG_TYPE_DEFAULT
#define kLogLevelWarning			60			//! Similar to LOG_WARNING
#define kLogLevelAssert 			70
#define kLogLevelRequire			80
#define kLogLevelError				90			//! Similar to LOG_ERR / OS_LOG_TYPE_ERROR
#define kLogLevelCritical			100			//! Similar to LOG_CRIT
#define kLogLevelAlert				110			//! Similar to LOG_ALERT
#define kLogLevelFault				115			//! Similar to OS_LOG_TYPE_FAULT
...
#define kLogLevelFlagSensitive		0x00800000	//! Use with ulog. Only logs if safe (i.e. os_log %{private}).
#define kLogLevelFlagThread			0x01000000	//! Use with ulog. Prints the name of the thread/dispatch queue calling ulog.
#define kLogLevelFlagPublic			0x02000000	//! Use with ulog. Treats log message a public (os_log %{public}).
#define kLogLevelFlagStackShot		0x04000000	//! Use with ulog. Triggers a stackshot.

// CoreUtils logging

#define kCULogConfig					"?.*:level=chatty,.*:flags=public"
#define kCULogLevelDefault			kLogLevelChatty
#define kCULogFlags					kLogFlags_Public
#define kCULogSubsystem				"com.apple.CoreUtils"
#define cu_ulog_define( NAME )		ulog_define_ex( kCULogSubsystem, NAME, kCULogLevelDefault, kCULogFlags, NULL, NULL )
...
#define kCUBundleIdentifier		CFSTR( "com.apple.CoreUtils" )
#define CUBundleIdentifier		@"com.apple.CoreUtils"
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Mac-style version numbers represented by 32-bit numbers (e.g. 1.2.3b4 -> 0x01236004).
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Builds a 32-bit Mac-style NumVersion value (e.g. NumVersionBuild( 1, 2, 3, kVersionStageBeta, 4 ) -> 1.2.3b4).
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Compares two NumVersion values and returns < 0 if A < B, 0 if A == B, or > 0 if A > B.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Converts source version components to an integer.
	@details	For example, source version 110.35 would be SourceVersionToInteger( 110, 35, 0 ) which is 1103500.
*/
#define SourceVersionToInteger( X, Y, Z )	( ( 10000 * (X) ) + ( 100 * (Y) ) + (Z) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Useful character sets.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macros for testing for hex chars and converting them to values and/or bytes.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macros for testing for octal chars and converting them to values and/or bytes.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Converts a byte to/from BSD (e.g. 0x50 -> 50).
*/		
#define	BCDByteToDecimal( X )	( ( ( ( (X) >> 4 ) & 0x0F ) * 10 ) + ( (X) & 0x0F ) )
#define	DecimalByteToBCD( X )	( ( ( (X) / 10 ) << 4 ) | ( (X) % 10 ) )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Increments a big endian integer.
*/
STATIC_INLINE void	BigEndianIntegerIncrement( uint8_t * _Nonnull inInteger, size_t inLen )
{
	while( inLen-- > 0 )
	{
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Increments a little endian integer.
*/
STATIC_INLINE void	LittleEndianIntegerIncrement( uint8_t * _Nonnull inInteger, size_t inLen )
{
	size_t		i;
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@details	Macros to convert between Celsius and Fahrenheit.
*/
#define CelsiusToFahrenheit( X )	( ( ( (X) * 9 ) / 5 ) + 32 )
#define FahrenheitToCelsius( X )	( ( ( (X) - 32 ) * 5 ) / 9 )

//---------------------------------------------------------------------------------------------------------------------------
/*!	@details	Macros to convert between dB attentuation values and linear volume levels.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macros to convert between floating point and fixed-point Q-format numbers.
	@details	See <http://en.wikipedia.org/wiki/Q_(number_format)> for details.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Handler to invoke when a failable operation completes.
	
	@param		inError		nil if successful. non-nil if an error occurred.
*/
#if( COMPILER_HAS_BLOCKS && COMPILER_OBJC )
	typedef void ( ^CUErrorHandler )( NSError * _Nullable inError );
#endif

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		IEEE Extended Unique Identifier (EUI) 48-bit. Same as a MAC address (MAC-48).
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		IEEE Extended Unique Identifier (EUI) 64-bit.
	@details	See <https://en.wikipedia.org/wiki/MAC_address>.
*/
typedef struct
{
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Tracks a simple state for an operation and it's final result.
*/
typedef int32_t		CUMultiState;
#define kCUMultiStateInvalid		0  // Invalid state (before work has started).
#define kCUMultiStateInProgress		1  // Work started, but hasn't reached its final state yet.
#define kCUMultiStateSkip			2  // Work should be skipped.
#define kCUMultiStateFailed			3  // Work failed.
#define kCUMultiStateSucceeded		4  // Work succeeded.
#define kCUMultiStateNo				5  // Boolean no/false for tri-states.
#define kCUMultiStateYes			6  // Boolean yes/true for tri-states.
#define kCUMultiStateCanceled		7  // Work canceled.
#define kCUMultiStateUserBase		10 // First user-defined state.

STATIC_INLINE bool	CUMultiStateContinue( CUMultiState inState )
{
	return( ( inState == kCUMultiStateSkip ) || ( inState == kCUMultiStateSucceeded ) );
}

STATIC_INLINE bool	CUMultiStateDone( CUMultiState inState )
{
	return( inState >= kCUMultiStateInProgress );
}

STATIC_INLINE const char *	CUMultiStateToString( CUMultiState inState )
{
	switch( inState )
	{
		case kCUMultiStateInvalid:		return( "Invalid" );
		case kCUMultiStateInProgress:	return( "InProgress" );
		case kCUMultiStateSkip:			return( "Skip" );
		case kCUMultiStateFailed:		return( "Failed" );
		case kCUMultiStateSucceeded:	return( "Succeeded" );
		case kCUMultiStateNo:			return( "No" );
		case kCUMultiStateYes:			return( "Yes" );
		case kCUMultiStateCanceled:		return( "Canceled" );
		default:						return( ( inState >= kCUMultiStateUserBase ) ? "User" : "?" );
	}
}

STATIC_INLINE const char *	CUMultiStateToShortString( CUMultiState inState )
{
	switch( inState )
	{
		case kCUMultiStateInvalid:		return( "?" );
		case kCUMultiStateInProgress:	return( "P" );
		case kCUMultiStateSkip:			return( "~" );
		case kCUMultiStateFailed:		return( "F" );
		case kCUMultiStateSucceeded:	return( "S" );
		case kCUMultiStateNo:			return( "N" );
		case kCUMultiStateYes:			return( "Y" );
		case kCUMultiStateCanceled:		return( "C" );
		default:						return( ( inState >= kCUMultiStateUserBase ) ? "U" : "-" );
	}
}

//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		IEEE Organizationally unique identifier, 24-bit.
	@details	See <https://en.wikipedia.org/wiki/Organizationally_unique_identifier>.
*/
typedef struct
{
	uint8_t		bytes[ 3 ];
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Block type for a commonly used block with a status parameter.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Compares memory so that the time it takes does not depend on the data being compared.
	@details	This is needed to avoid certain timing attacks in cryptographic software.
*/
STATIC_INLINE int	memcmp_constant_time( const void * _Nullable inA, const void * _Nullable inB, size_t inLen )
{
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Zeros memory in a way that prevents the compiler from optimizing it out (as it could with memset).
	@details	This is needed for cases such as clearing a buffer containing a cryptographic key.
*/
#if( ( defined( __STDC_WANT_LIB_EXT1__ ) && ( __STDC_WANT_LIB_EXT1__ >= 1 ) && !TARGET_KERNEL ) || ( QNX_VERSION >= 660 ) )
STATIC_INLINE void	MemZeroSecure( void * _Nullable inPtr, size_t inLen )
{
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Macros for using a mutex with minimal requirements.
*/
#if( TARGET_OS_EFI )
	#define MinimalMutexDefine( NAME )					static EFI_LOCK	 NAME = EFI_INITIALIZE_LOCK_VARIABLE( EFI_TPL_HIGH_LEVEL )
	#define MinimalMutexEnsureInitialized( NAME )		do {} while( 0 )
	#define MinimalMutexEnsureFinalized( NAME )			do {} while( 0 )
	#define MinimalMutexLock( NAME )					EfiAcquireLock( &NAME )
	#define MinimalMutexUnlock( NAME )					EfiReleaseLock( &NAME )
#elif( TARGET_OS_DARWIN_KERNEL )
	...
	#define MinimalMutexDefineRecursive( NAME )			static pthread_mutex_t NAME = PTHREAD_RECURSIVE_MUTEX_INITIALIZER
	#define MinimalMutexEnsureInitialized( NAME )		do {} while( 0 )
	#define MinimalMutexEnsureFinalized( NAME )			do {} while( 0 )
...
#if( COMPILER_OBJC )
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Return YES if the two objects have equal content, NO otherwise.
	@details	Note: If both are nil, returns YES. If one is nil and the other is not, returns NO.
*/
STATIC_INLINE BOOL	NSObjectsAreEqual( id _Nullable inLeft, id _Nullable inRight )
{
	if( inLeft == inRight ) return( true );
	if( ( inLeft != nil ) != ( inRight != nil ) ) return( false );
	return( [inLeft isEqual:inRight] );
}
#endif

#if( TARGET_OS_DARWIN_REAL && !TARGET_OS_DARWIN_KERNEL )
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Wrapper around notify_get_state to return the value inline to make it easier to call.
*/
STATIC_INLINE uint64_t notify_get_state_inline( int inToken, OSStatus *outErr )
{
	uint64_t state = 0;
	OSStatus err = (OSStatus) notify_get_state( inToken, &state );
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Convert between Time96 and floating-point seconds values.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Convert between Time96 and NTP 32.32 values.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Convert between Time96 fractional seconds and nanoseconds.
*/
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Adds a fractional seconds (1/2^64 units) value to a time.
*/
STATIC_INLINE void	Time96_AddFrac( Time96 * _Nonnull inTime, uint64_t inFrac )
{
	uint64_t		frac;
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Adds one time to another time.
*/
STATIC_INLINE void	Time96_Add( Time96 * _Nonnull inTime, const Time96 * _Nonnull inAdd )
{
	uint64_t		frac;
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Subtracts one time from another time.
*/
STATIC_INLINE void	Time96_Sub( Time96 * _Nonnull inTime, const Time96 * _Nonnull inSub )
{
	uint64_t	frac;
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		ANSI Escape Sequences for use with printf/etc to terminal windows.
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@brief		Green thread implementation for lightweight, stackless, cooperatively-scheduled threads.
...
typedef GState ( *GreenThreadFunc )( GreenThread * _Nonnull inThread, void * _Nullable inArg );
struct GreenThread
{
	int							pc;
	GreenTopThread * _Nonnull	topThread;
};

struct GreenTopThread
{
	GreenThread					gt;
	GreenThreadFunc _Nonnull	func;
};
